...
+ These functions need to be reviewed more closely to make sure that all the various possible
  floating point errors are being checked for and handled appropriately. We do check errno in a
  few places, but we're not sure we're doing that properly. In any case there are plenty of
...
+ We need to think some more about how we want to report errors. Right now we're just throwing a
  generic Entity::Error object with a reasonably appropriate message. Do we want to distinguish
  between range errors, domain errors, underflow, overflow, etc?

+ We should improve my handling of pi and e. Should we create a constants library for clac and
  store carefully prepared values of various constants there? clac users will, after all, need
  some of these values too.
*/
