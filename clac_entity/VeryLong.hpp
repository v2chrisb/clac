...
/*!
    This class intends to make manipulating extended precision integers natural and easy. It is
    not especially efficient although it uses "reasonable" algorithms (at least that is the
    intent). The primary goal is to make using extended precision integers as much like using
    the built in integer types as possible.

    VeryLong integers have as many digits as required, limited only by available memory. This
    implementation uses STL vectors to manage the digits. It stores the VeryLongs using a
    signed/magnitude representation. It does not use 2's complement for representing negative
    values. This is significant if you access a VeryLong on a bit by bit basis using get_bit or
    put_bit. This implementation insures that zero is always positive. Thus zero has only one
    representation and comparisons against zero are well defined.
    ...
    long. This is unlikely to be an issue in practice (for example a 512 bit number requires
    only 32 storage units on a 32 bit machine.

    The following statement describes the exception safety of this class.
    ...
    exception enter an undefined state and should not be used in any further operations of any
    kind. However, such objects are still destructable and the destructor will correctly recover
    all resources that were being used by such objects.</p></li>
    </ol>
    ...
    require making modifications to the class that would reduce its performance to an
...
class VeryLong {
    ...
    /*! Writes the number as a sequence of digit characters with no special formatting. This
        function currently ignores the settings in the ios part of the given stream.
    */
...
public:
    ...
    /*! For example if the argument is "123456", this constructor would initialize the VeryLong
        ...
        "" would all initialize the VeryLong to zero.
    */
