...
current implementation allows for a maximum of 32 bits. The actual number of bits being used is
controlled by the user and available through Global::get_bit_count. Every time an operation is
applied to a BinaryEntity, the most significant bits (which are unused) are masked to zero by
